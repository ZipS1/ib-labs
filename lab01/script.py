import sqlite3
import os
from hashlib import md5

# AES implementation (educational version)

s_box = (
    ,
)

inv_s_box = (
    ,
)

def sub_bytes(state):
    for i in range(4):
        for j in range(4):
            state[i][j] = s_box[state[i][j]]

def inv_sub_bytes(state):
    for i in range(4):
        for j in range(4):
            state[i][j] = inv_s_box[state[i][j]]

def shift_rows(s):
    s[0][1], s[1][1], s[2][1], s[3][1] = s[1][1], s[2][1], s[3][1], s[0][1]
    s[0][2], s[1][2], s[2][2], s[3][2] = s[2][2], s[3][2], s[0][2], s[1][2]
    s[0][3], s[1][3], s[2][3], s[3][3] = s[3][3], s[0][3], s[1][3], s[2][3]

def inv_shift_rows(s):
    s[0][1], s[1][1], s[2][1], s[3][1] = s[3][1], s[0][1], s[1][1], s[2][1]
    s[0][2], s[1][2], s[2][2], s[3][2] = s[2][2], s[3][2], s[0][2], s[1][2]
    s[0][3], s[1][3], s[2][3], s[3][3] = s[1][3], s[2][3], s[3][3], s[0][3]

def gmul(a, b):
    p = 0
    for _ in range(8):
        if b & 1:
            p ^= a
        a <<= 1
        if a & 0x100:
            a ^= 0x11B
        b >>= 1
    return p

def mix_columns(state):
    for i in range(4):
        s0 = state[i][0]
        s1 = state[i][1]
        s2 = state[i][2]
        s3 = state[i][3]

        state[i][0] = gmul(s0, 2) ^ gmul(s1, 3) ^ s2 ^ s3
        state[i][1] = s0 ^ gmul(s1, 2) ^ gmul(s2, 3) ^ s3
        state[i][2] = s0 ^ s1 ^ gmul(s2, 2) ^ gmul(s3, 3)
        state[i][3] = gmul(s0, 3) ^ s1 ^ s2 ^ gmul(s3, 2)

def inv_mix_columns(state):
    for i in range(4):
        s0 = state[i][0]
        s1 = state[i][1]
        s2 = state[i][2]
        s3 = state[i][3]

        state[i][0] = gmul(s0, 0x0e) ^ gmul(s1, 0x0b) ^ gmul(s2, 0x0d) ^ gmul(s3, 0x09)
        state[i][1] = gmul(s0, 0x09) ^ gmul(s1, 0x0e) ^ gmul(s2, 0x0b) ^ gmul(s3, 0x0d)
        state[i][2] = gmul(s0, 0x0d) ^ gmul(s1, 0x09) ^ gmul(s2, 0x0e) ^ gmul(s3, 0x0b)
        state[i][3] = gmul(s0, 0x0b) ^ gmul(s1, 0x0d) ^ gmul(s2, 0x09) ^ gmul(s3, 0x0e)

def add_round_key(state, round_key):
    for i in range(4):
        for j in range(4):
            state[i][j] ^= round_key[i][j]

def bytes_to_matrix(byte_data):
    return [list(byte_data[i:i+4]) for i in range(0, len(byte_data), 4)]

class AES:
    def __init__(self, key):
        self.round_keys = [bytes_to_matrix(key)] * 11  # Упрощенное расширение ключа

    def encrypt_block(self, plaintext):
        state = bytes_to_matrix(plaintext)
        add_round_key(state, self.round_keys[0])

        for i in range(1, 10):
            sub_bytes(state)
            shift_rows(state)
            mix_columns(state)
            add_round_key(state, self.round_keys[i])

        sub_bytes(state)
        shift_rows(state)
        add_round_key(state, self.round_keys[10])

        return bytes(sum(state, []))

    def decrypt_block(self, ciphertext):
        state = bytes_to_matrix(ciphertext)
        add_round_key(state, self.round_keys[10])
        inv_shift_rows(state)
        inv_sub_bytes(state)

        for i in range(9, 0, -1):
            add_round_key(state, self.round_keys[i])
            inv_mix_columns(state)
            inv_shift_rows(state)
            inv_sub_bytes(state)

        add_round_key(state, self.round_keys[0])
        return bytes(sum(state, []))

def encrypt(key, plaintext):
    iv = os.urandom(16)
    cipher = AES(key)
    pad_len = 16 - (len(plaintext) % 16)
    padded = plaintext + bytes([pad_len] * pad_len)
    ciphertext = b''
    prev = iv

    for i in range(0, len(padded), 16):
        block = bytes([padded[i+j] ^ prev[j] for j in range(16)])
        encrypted = cipher.encrypt_block(block)
        ciphertext += encrypted
        prev = encrypted

    return iv + ciphertext

def decrypt(key, ciphertext):
    iv = ciphertext[:16]
    ciphertext = ciphertext[16:]
    cipher = AES(key)
    plaintext = b''
    prev = iv

    for i in range(0, len(ciphertext), 16):
        block = ciphertext[i:i+16]
        decrypted = cipher.decrypt_block(block)
        plaintext += bytes([decrypted[j] ^ prev[j] for j in range(16)])
        prev = block

    pad_len = plaintext[-1]
    return plaintext[:-pad_len]

def create_db():
    conn = sqlite3.connect('users.db')
    c = conn.cursor()
    c.execute('''CREATE TABLE IF NOT EXISTS users
                 (id INTEGER PRIMARY KEY, username TEXT, password BLOB)''')
    conn.commit()
    conn.close()

def insert_user(username, password, key):
    encrypted = encrypt(key, password.encode())
    conn = sqlite3.connect('users.db')
    c = conn.cursor()
    c.execute("INSERT INTO users (username, password) VALUES (?, ?)",
              (username, encrypted))
    conn.commit()
    conn.close()

def get_user(username, key):
    conn = sqlite3.connect('users.db')
    c = conn.cursor()
    c.execute("SELECT password FROM users WHERE username=?", (username,))
    row = c.fetchone()
    conn.close()
    if row:
        return decrypt(key, row[0]).decode()
    return None

if __name__ == "__main__":
    secret_key = md5(b'my-secret-key').digest()
    create_db()

    # Работа с базой данных
    insert_user('bob', 'supersecretpasswd', secret_key)
    password = get_user('bob', secret_key)
    print(f"Расшифрованный пароль: {password}")
